...
export default function Navbar() {
  ...
  return (
    ...
    >
      <nav
        className="nav-pill flex items-center justify-between max-w-6xl mx-auto px-6 py-3"
          ...
          {NAV_LINKS.map((link) => (
            <li key={link.path}>
              <Link
                ...
                style={{
                  color: isActive(link.path) ? '#e8462a' : 'rgba(255,255,255,0.75)',
                  border: isActive(link.path) ? '1px solid rgba(232,70,42,0.4)' : '1px solid transparent',
                }}
              >
                {link.label}
              </Link>
      ...
      {mobileOpen && (
        <div
          ...
          style={{ background: 'rgba(7, 29, 56, 0.98)', border: '1px solid rgba(255,255,255,0.08)', backdropFilter: 'blur(20px)' }}
        >
          <ul className="py-4">
            {NAV_LINKS.map((link) => (
              <li key={link.path}>
                <Link
                  ...
                  style={{ color: isActive(link.path) ? '#e8462a' : 'rgba(255,255,255,0.75)' }}
                >
                  {link.label}
                </Link>
              </li>
            ...
            <li className="px-6 pt-3 pb-2">
              <Link to="/contact" className="btn-primary px-5 py-2 text-sm w-full justify-center">
                Contact Us
              </Link>
            </li>
          </ul>
